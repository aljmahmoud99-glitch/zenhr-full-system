...
const frontendPort = Number(process.env.FRONTEND_PORT || 5020);
...
const out = path.join(__dirname, "phase-10-browser-results.json");
...
async function inspect(page, key, route, width = 1366) {
  ...
  results.pages[key] = evalResult.result?.result?.value || {};
}
...
async function main() {
  ...
  try {
    ...
    chrome = spawn(chromePath, [
      ...
      "--user-data-dir=" + path.join(__dirname, `.chrome-phase-10-${Date.now()}`),
      "--headless=new",
      "--disable-gpu",
      "--disable-extensions",
    ...
    await setSession(page, logins.payroll, "en", "dark");
    await inspect(page, "payroll", "/app/payroll/runs");
    ...
    await inspect(page, "employeeMobile", "/app/notifications", 390);

    await setSession(page, logins.recruiter, "en", "dark");
    await inspect(page, "recruitment", "/app/recruitment");
