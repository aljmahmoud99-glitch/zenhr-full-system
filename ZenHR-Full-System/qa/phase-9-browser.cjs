...
const frontendPort = Number(process.env.FRONTEND_PORT || 5019);
...
const out = path.join(__dirname, "phase-9-browser-results.json");
...
async function connect(port) {
  ...
  const send = (method, params = {}) => new Promise((resolve) => {
    ...
    const timer = setTimeout(() => {
      ...
      resolve({ timeout: true, id: callId, method });
    }, 10000);
...
async function main() {
  ...
  try {
    ...
    const logins = { hr: await login("hr"), payroll: await login("payroll"), employee: await login("employee"), recruiter: await login("recruiter") };
    const chromePath = findChrome();
    if (!chromePath) throw new Error("Chrome/Edge executable not found");
    const port = 13100 + Math.floor(Math.random() * 500);
    chrome = spawn(chromePath, [
      `--remote-debugging-port=${port}`,
      "--user-data-dir=" + path.join(__dirname, `.chrome-phase-9-${Date.now()}`),
      "--headless=new",
      "--disable-gpu",
      "--disable-extensions",
    ...
    await inspect(page, "hrPayrollOperations", "/app/payroll-attendance");
    await setSession(page, logins.payroll);
    await inspect(page, "payrollRun", "/app/payroll");
