...
async function main() {
  ...
  // Null out employee_id on all users first (avoids FK violation on admin)
  await q('UPDATE users SET employee_id = NULL');
  ...
  await q('DELETE FROM job_descriptions WHERE company_id <> $1', [COMPANY_ID]);
  await q('DELETE FROM salary_components WHERE company_id <> $1', [COMPANY_ID]);
  await q('DELETE FROM salary_component_definitions WHERE company_id <> $1', [COMPANY_ID]);
  await q('DELETE FROM assets WHERE company_id <> $1', [COMPANY_ID]);
  await q('DELETE FROM career_paths WHERE company_id <> $1', [COMPANY_ID]);
  await q('DELETE FROM activity_logs WHERE company_id <> $1', [COMPANY_ID]);
  await q('DELETE FROM system_configurations WHERE company_id <> $1', [COMPANY_ID]);
  await q('DELETE FROM payroll_runs WHERE company_id <> $1', [COMPANY_ID]);
  await q('DELETE FROM leave_policies WHERE company_id <> $1', [COMPANY_ID]);
  await q('DELETE FROM users WHERE company_id <> $1', [COMPANY_ID]);
  ...
  await q(`
    INSERT INTO leave_policies (company_id, leave_type, name_ar, name_en, days_per_year, max_carry_forward_days, is_paid, is_active)
    VALUES
      ($1, 'annual',    'إجازة سنوية',   'Annual Leave',    21, 10, true,  true),
      ($1, 'sick',      'إجازة مرضية',   'Sick Leave',      14,  0, true,  true),
      ($1, 'emergency', 'إجازة طارئة',   'Emergency Leave',  3,  0, true,  true)
